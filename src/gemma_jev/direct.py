...
from .core import answer_slots, build_prompt, digest, encode_text, slot_separators, softmax, synchronize

PROMPT_VERSION = "gemma-jev-letters-v1"
...
def encode_prompt(tokenizer, row: dict, style: str, shots: int, max_tokens: int):
    ...
    slots, separator = answer_slots(tokenizer, prompt, len(row["options"]), slot_separators(style))
